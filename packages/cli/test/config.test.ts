...
import { getFileMode, readConfig, writeConfig } from "../src/config.js";
...
describe("CLI config storage", () => {
  ...
  it("returns an empty config when the file does not exist", async () => {
    await expect(readConfig(join(tmpdir(), "neptune-missing-config.json"))).resolves.toEqual({});
  });
  ...
  it("migrates a legacy local config when the Neptune config is missing", async () => {
    ...
    try {
      ...
      await expect(readConfig(configPath, oldConfigPath)).resolves.toMatchObject({
        auth: {
          user: { email: "user@example.com" }
        }
      });
