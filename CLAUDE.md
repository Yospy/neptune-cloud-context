...
Neptune is shared project context for AI coding agents (Codex, Claude Code). Agents coordinate through structured markdown context stored in Supabase, called through an MCP server and a backend API. There is intentionally no product UI in V1.
...
```
Codex / Claude Code  →  MCP server  →  SDK  →  Backend API (HTTPS)  →  Supabase (Auth + Postgres)
```
